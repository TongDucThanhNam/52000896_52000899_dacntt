...
export default function ProductDetail({product}: ProductDetailProps) {
    ...
    const handleAddToCart = () => {
        ...
        const newItem: CartItem = {
            ...
            quantity,
            imageUrl: variantChosen.variantImageUrl && variantChosen.variantImageUrl !== "https://placehold.co/50" ? variantChosen.variantImageUrl : product.imageUrls[0],
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <Card className="space-y-8 px-6 py-8">
                    <div className="space-y-4">
                        ...
                        <h2 className="text-2xl font-semibold text-gray-700">Hãng: {product.productBrand}</h2>
                        <p className="text-3xl font-bold text-red-600">{priceVietNamDongformetter(productPrice)}</p>
                        <div className="flex items-center space-x-4 text-sm text-gray-600">
                            <span>Số lượt xem: {product.productTotalViews}</span>
                            <span>|</span>
                            <span>Đánh giá: {product.productAvgRating?.toFixed(1) ?? "N/A"} ⭐</span>
                        </div>
                    </div>
                    ...
                    {variantChosen && (
                        <div className="flex items-center space-x-4 bg-gray-100 p-4 rounded-lg">
                            ...
                            />
                            <p className="text-lg font-medium text-gray-800">{variantChosen.variantName}</p>
                        </div>
                    )}
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="flex items-center">
                            <Button variant="noShadow" size="icon" onClick={() => handleQuantityChange(-1)}
                    ...
                    <Accordion type="multiple" className="space-y-4">
                        ...
                        <AccordionItem value="return" className="border rounded-lg">
                            <AccordionTrigger className="px-4 py-3 hover:bg-gray-50">Tôi có được trả hàng không
                                ?</AccordionTrigger>
                            <AccordionContent className="px-4 py-3 bg-gray-50">
                                <p className="text-gray-700">Bạn có thể trả hàng trong vòng 7 ngày.</p>
                            </AccordionContent>
                        </AccordionItem>
                    </Accordion>
                </Card>
