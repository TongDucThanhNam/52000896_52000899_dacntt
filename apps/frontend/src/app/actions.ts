...
export async function createProduct(
  productData: Omit<Product, "id" | "productId">,
) {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/variants`,
      {
        method: "POST",
        headers: {
...
async function updateVariant(variantId: string, variantData: Variant) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/variants/${variantId}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
async function createVariant(productId: string, variantData: Variant) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/${productId}/variants`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function checkoutCart(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: "Có lỗi xảy ra khi thanh toán. Vui lòng thử lại.",
      };
    }
...
export default async function fetchTransactionById(
  id: string,
): Promise<Transaction> {
  const res = await fetch(
...
export async function trackInteractionAction(interaction: Interaction) {
  try {
    ...
    const response = await fetch(
      `${process.env.BACKEND_URL}/api/interactions`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
