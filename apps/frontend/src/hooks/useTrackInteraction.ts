...
type InteractionWithoutUserIdAndType = Omit<
  Interaction,
...
export function useTrackInteraction() {
  ...
  const trackViewInteraction = (
    interaction: InteractionWithoutUserIdAndType,
  ) => {
    return trackInteraction({ ...interaction, interactionType: "view" });
  };

  const trackCartInteraction = (
    interaction: InteractionWithoutUserIdAndType,
  ) => {
    return trackInteraction({ ...interaction, interactionType: "cart" });
  };

  const trackPurchaseInteraction = (
    interaction: InteractionWithoutUserIdAndType,
  ) => {
