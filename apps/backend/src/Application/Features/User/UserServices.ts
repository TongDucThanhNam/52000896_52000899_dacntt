...
class UserServices implements IUserServices {
  ...
  async createUser(data: any): Promise<any> {
    try {
      ...
      if (existingUser && existingUser.length > 0) {
        return new CoreException(400, "Người dùng đã tồn tại");
      }
  ...
  async getUserById(data: any): Promise<any> {
    try {
      ...
      const user = await this.unitOfWork.userRepository.getUserById(
        data,
        queryData,
      );
      return user;
  ...
  async getUserPreferences(data: any): Promise<any> {
    try {
      ...
      const preferences: any =
        await this.unitOfWork.preferenceRepository.getPreferencesByUserId(
          userId,
          restData,
        );
      return preferences;
  ...
  async updateUser(data: any): Promise<any> {
    try {
      ...
      if (
        ...
        userData.userDateOfBirth instanceof Date
      ) {
        userData.userDateOfBirth = userData.userDateOfBirth.toISOString();
      }

      const user = await this.unitOfWork.userRepository.updateUserById(
        userId,
        userData,
      );
      return user;
  ...
  async updateUserPreferences(data: any): Promise<any> {
    try {
      const { userId, ...preferenceData } = data;
      const preferences =
        await this.unitOfWork.preferenceRepository.updatePreferenceById(
          userId,
          preferenceData,
        );
      return preferences;
  ...
  async loginUser(data: any): Promise<any> {
    try {
      ...
      const users: any =
        await this.unitOfWork.userRepository.getAllUsers(queryData);

      if (!users || users.length === 0) {
        // throw new Error('User not found');
        return new CoreException(401, "Người dùng không tồn tại");
      }
      ...
      if (!isValid) {
        ...
        return new CoreException(401, "Mật khẩu không chính xác");
      }
